...
{
    /// <summary>
    ///     A custom <see cref="IDocumentationProvider" /> that reads the API
        ...
        IModelDocumentationProvider
    {
            ...
            HttpParameterDescriptor parameterDescriptor)
        {
            ...
            {
                var methodNode =
                    GetMethodNode(reflectedParameterDescriptor.ActionDescriptor);
                if (methodNode != null)
                {
                    var parameterName =
                        reflectedParameterDescriptor.ParameterInfo.Name;
                    var parameterNode =
        ...
        {
            var memberName = string.Format(CultureInfo.InvariantCulture,
                "{0}.{1}", GetTypeName(member.DeclaringType), member.Name);
            var expression = member.MemberType == MemberTypes.Field
                ? FieldExpression
                : PropertyExpression;
            var selectExpression = string.Format(CultureInfo.InvariantCulture,
        ...
        private XPathNavigator GetMethodNode(
            HttpActionDescriptor actionDescriptor)
        {
            ...
            if (reflectedActionDescriptor != null)
            {
                var selectExpression =
                    string.Format(CultureInfo.InvariantCulture, MethodExpression,
        ...
        private static string GetMemberName(MethodInfo method)
        {
            var name = string.Format(CultureInfo.InvariantCulture, "{0}.{1}",
                GetTypeName(method.DeclaringType), method.Name);
            var parameters = method.GetParameters();
            if (parameters.Length != 0)
            {
                var parameterTypeNames =
                    parameters.Select(param => GetTypeName(param.ParameterType))
                        .ToArray();
        ...
        private static string GetTagValue(XPathNavigator parentNode,
            string tagName)
        {
            if (parentNode != null)
            {
        ...
        private XPathNavigator GetTypeNode(Type type)
        {
            ...
            var selectExpression = string.Format(CultureInfo.InvariantCulture,
                TypeExpression, controllerTypeName);
            return _documentNavigator.SelectSingleNode(selectExpression);
        }
        ...
        private static string GetTypeName(Type type)
        {
            ...
            if (type.IsGenericType)
            {
                ...
                var argumentTypeNames =
                    genericArguments.Select(t => GetTypeName(t)).ToArray();
