...
namespace Aegis.Monitor.Filter.Areas.HelpPage
{
    ...
    public class ObjectGenerator
    {
        ...
        private readonly SimpleTypeObjectGenerator SimpleObjectGenerator =
            new SimpleTypeObjectGenerator();

        /// <summary>
        ///     Generates an object for a given type. The type needs to be public,
        ///     have a public default constructor and settable public properties/fields.
        ///     Currently it supports the following types: Simple types: <see cref="int" />
        ///     , <see cref="string" />, <see cref="Enum" />, <see cref="DateTime" />,
        ///     <see cref="Uri" />, etc. Complex types: POCO types. Nullables:
        ///     <see cref="Nullable{T}" />. Arrays: arrays of simple types or complex
        ///     types. Key value pairs: <see cref="KeyValuePair{TKey,TValue}" />
        ///     Tuples: <see cref="Tuple{T1}" />, <see cref="Tuple{T1,T2}" />, etc
        ///     Dictionaries: <see cref="IDictionary{TKey,TValue}" /> or anything deriving
        ///     from <see cref="IDictionary{TKey,TValue}" />. Collections:
        ///     <see cref="IList{T}" />, <see cref="IEnumerable{T}" />,
        ///     <see cref="ICollection{T}" />, <see cref="IList" />,
        ///     <see cref="IEnumerable" />, <see cref="ICollection" /> or anything deriving
        ///     from <see cref="ICollection{T}" /> or <see cref="IList" />. Queryables:
        ///     <see cref="IQueryable" />, <see cref="IQueryable{T}" />.
        /// </summary>
            ...
            Dictionary<Type, object> createdObjectReferences)
        {
            try
            {
                ...
                if (type.IsArray)
                {
                    return GenerateArray(type, DefaultCollectionSize,
                        createdObjectReferences);
                }

                if (type.IsGenericType)
                {
                    return GenerateGenericType(type, DefaultCollectionSize,
                        createdObjectReferences);
                }
            ...
            Dictionary<Type, object> createdObjectReferences)
        {
            ...
            if (genericArguments.Length == 2)
            {
                ...
                {
                    return GenerateDictionary(type, collectionSize,
                        createdObjectReferences);
                }
            }
        ...
        private static object GenerateTuple(Type type,
            Dictionary<Type, object> createdObjectReferences)
        {
            ...
            {
                parameterValues[i] =
                    objectGenerator.GenerateObject(genericArgs[i],
                        createdObjectReferences);
            ...
            Dictionary<Type, object> createdObjectReferences)
        {
            ...
            var objectGenerator = new ObjectGenerator();
            var keyObject = objectGenerator.GenerateObject(typeK,
                createdObjectReferences);
            var valueObject = objectGenerator.GenerateObject(typeV,
                createdObjectReferences);
            ...
            var result = Activator.CreateInstance(keyValuePairType, keyObject,
                valueObject);
            return result;
        }

        private static object GenerateArray(Type arrayType, int size,
            Dictionary<Type, object> createdObjectReferences)
        {
            ...
            {
                var element = objectGenerator.GenerateObject(type,
                    createdObjectReferences);
            ...
            Dictionary<Type, object> createdObjectReferences)
        {
            ...
            var addMethod = dictionaryType.GetMethod("Add") ??
                            dictionaryType.GetMethod("TryAdd");
            var containsMethod = dictionaryType.GetMethod("Contains") ??
                                 dictionaryType.GetMethod("ContainsKey");
            var objectGenerator = new ObjectGenerator();
            for (var i = 0; i < size; i++)
            {
                var newKey = objectGenerator.GenerateObject(typeK,
                    createdObjectReferences);
                if (newKey == null)
                {
                ...
                var containsKey =
                    (bool) containsMethod.Invoke(result, new[] {newKey});
                if (!containsKey)
                {
                    var newValue = objectGenerator.GenerateObject(typeV,
                        createdObjectReferences);
        ...
        private static object GenerateNullable(Type nullableType,
            Dictionary<Type, object> createdObjectReferences)
        {
        ...
        private static object GenerateComplexObject(Type type,
            Dictionary<Type, object> createdObjectReferences)
        {
            object result = null;
        ...
        private static void SetPublicProperties(Type type, object obj,
            Dictionary<Type, object> createdObjectReferences)
        {
            var properties =
            ...
            foreach (var property in properties)
            {
                ...
                {
                    var propertyValue =
                        objectGenerator.GenerateObject(property.PropertyType,
                            createdObjectReferences);
        ...
        private static void SetPublicFields(Type type, object obj,
            Dictionary<Type, object> createdObjectReferences)
        {
            var fields =
            ...
            {
                var fieldValue = objectGenerator.GenerateObject(
                    field.FieldType, createdObjectReferences);
        ...
        {
            private static readonly Dictionary<Type, Func<long, object>>
                DefaultGenerators = InitializeGenerators();
            ...
            private static Dictionary<Type, Func<long, object>>
                        ...
                        index =>
                        {
                            return
                                new Uri(string.Format(
                                    ...
                                    "http://webapihelppage{0}.com", index));
                        }
                    }
                };
            }
