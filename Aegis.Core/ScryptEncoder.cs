...
namespace Aegis.Core
{
    ...
    public class ScryptEncoder
    {
        ...
        /// Hash a password using the scrypt scheme.
        /// </summary>
        public static byte[] EncodeRaw(byte[] passwordBytes, byte[] saltBytes, int iterationCount, int blockSize, int threadCount)
        {
            var N = (long)Math.Pow(2, iterationCount);
            var r = blockSize;
            var p = threadCount;

            return CryptoScrypt(passwordBytes, saltBytes, N, r, p);
        }
